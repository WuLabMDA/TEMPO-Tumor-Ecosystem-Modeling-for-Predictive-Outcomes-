% $Header: svn://.../trunk/AMIGO2R2016/IMC_BC_Analysis/Chemotherapy/Chemo_Patient_NT009.m 2410 2015-12-07 13:58:57Z evabalsa $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
inputs.pathd.results_folder='imc_bc_chemotherapy_1';         % Folder to keep results (in Results) for a given problem          
inputs.pathd.short_name='imc_chemo_p1';                      % To identify figures and reports for a given problem   
...
inputs.model.par=[0.0915 75 0.18 0.22 0.082 0.145 0.52 0.156 0.245 0.148 0.095 0.138 0.142 0.155 0.15...   % CC parameters
                 0.025 50 0.40 0.85 0.018 0.11 0.095 0.15 0.25 0.11 0.219...   % Treg parameters
                 5.15 50 0.98 0.92 0.98 0.0905 0.0108 0.09035 0.00105... % Teff parameters
                 2.10 50 0.65 0.95 0.11 0.13 0.0155 0.0025...   % M parameters
                 1.595 50 0.155 0.0015...      % EC parameters
                 10.85 100 3.6 0.1005 0.10075 0.002]; % F parameters % These values may be updated during optimization  
...
%==================================
 ...
 inputs.exps.exp_y0{iexp}=[73.50 1.78 6.31 8.99 2.49 6.94];  %Initial conditions for each experiment          
 inputs.exps.t_f{iexp}= 9;  %Experiments duration
 end
...
%==================================                                                            
 inputs.exps.n_s{1}=3;                                % [] Number of sampling times for each experiment.
%inputs.exps.n_s{2}=25;                                %    Optative input. By default "continuous" measurements are assumed.
 inputs.exps.t_s{1}=[0 1 9];                      % [] Sampling times for each experiment, by default equidistant
% inputs.exps.t_s{2}=[0 5 7 ...];                      % [] Sampling times for each experiment, by default equidistant
 ...
 inputs.exps.exp_data{1}=[                              
		73.50 1.78 6.31 8.99 2.49 6.94 
        %1.06 9.03 48.02 23.48 4.85 13.56 
        29.20 0.54 8.65 2.54 0.57 59.19 
		];
...
inputs.PEsol.global_theta_guess=[0.0915 0.15 0.025 0.219 5.15 0.00105 2.10 0.0025 1.595 0.0015 10.85 0.002];
