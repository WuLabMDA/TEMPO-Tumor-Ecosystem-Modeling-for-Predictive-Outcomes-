...
close all;
rng('default');
Patient_NT022   % Calls the script with the inputs: 
                           % Model
...
if isfield(results, 'sim') && isfield(results.sim, 'obs')
   ...
   csv_filename = fullfile('V:\dkmarri\Mathematical_Model\AMIGO2_R2019c\AMIGO2_R2019c\IMC_BC_Analysis\Chemotherapy', 'PCR_Patient_022_simulation_results.csv');
...
% % Example of plotting experimental data with larger markers
% figure;
% hold on;
% plot(inputs.exps.t_s{1}, inputs.exps.exp_data{1}(:, 1), 'ro', 'MarkerSize', 24, 'LineWidth', 2); % Red circle markers
% plot(inputs.exps.t_s{1}, inputs.exps.exp_data{1}(:, 2), 'bs', 'MarkerSize', 24, 'LineWidth', 2); % Blue square markers
