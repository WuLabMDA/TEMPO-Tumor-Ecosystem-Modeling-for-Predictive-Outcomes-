% $Header: svn://.../trunk/AMIGO2R2016/IMC_BC_Analysis/Chemotherapy/Chemo_Patient_NT067.m 2410 2015-12-07 13:58:57Z evabalsa $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
inputs.pathd.results_folder='imc_bc_chemotherapy_15';         % Folder to keep results (in Results) for a given problem          
inputs.pathd.short_name='imc_chemo_p15';                      % To identify figures and reports for a given problem   
...
inputs.model.par_names=char('Gr_cc','K_cc','alpha_ar','alpha_ki67','k_cd68','k_cd163','k_vimentin','k_sma','k_caveolin1','k_cd31c','k_pd1','k_cd8','k_gzmb','kcarb_cc', 'mu_cc',...
                            ...
                            'Gr_teff','K_teff','alpha_ox40','alpha_icos','alpha_cd3','k_cd45','k_pdl1', 'kcarb_teff','mu_teff',...
                            'Gr_m','K_m','alpha_cd11c','alpha_cd68m', 'k_cd31m', 'k_pdpn', 'kcarb_m','mu_m',...
...
inputs.model.par=[2.1915 85 0.086 0.183 0.035 0.111 0.435 0.130 0.330 0.132 0.043 0.127 0.118 0.085 0.015...   % CC parameters
                 0.065 10 0.633 0.638 0.090 0.11 0.060 0.410 0.127 0.102 0.105...   % Treg parameters
                 0.0155 45 0.120 0.100 0.484 0.683 0.101 0.118 0.120... % Teff parameters
                 0.010 15 0.931 0.875 0.082 0.072 0.135 0.125...   % M parameters
                 0.091 10 0.099 0.15...      % EC parameters
                 1.85 45 3.199 0.0446 0.148 0.01]; % F parameters % These values may be updated during optimization  
...
inputs.exps.exp_data{1}=[                              
		51.98 2.04 21.54 2.93 2.82 18.69 
        %18.39 6.84 16.77 15.87 2.71 39.42 
        61.13 0.56 1.42 0.92 0.91 3.50 
		];
...
inputs.PEsol.global_theta_guess=[2.1915 0.015 0.065 0.105 0.0155 0.120 0.010 0.125 0.091 0.15 1.85 0.01];

inputs.PEsol.global_theta_max=10*ones(1,12);  % Maximum allowed values for the paramters
inputs.PEsol.global_theta_min=0*ones(1,12); % Minimum allowed values for the paramters
