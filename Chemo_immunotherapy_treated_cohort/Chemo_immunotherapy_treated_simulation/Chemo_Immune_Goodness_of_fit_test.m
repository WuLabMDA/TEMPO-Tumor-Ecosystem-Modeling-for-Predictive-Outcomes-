...
%% ---------------- Load Data ----------------
data = readtable('Chemo_Immune_Goodness_of_fit_test.csv');
...
for t = 1:length(timePoints)
    ...
    for c = 1:length(cellTypes)
        ...
        nexttile;
        scatter(meas, pred, 80, 'filled', 'MarkerFaceColor', celltype_colors(c,:)); hold on;
...
for t = 1:length(timePoints)
    ...
    for c = 1:length(cellTypes)
        idx = (all_labels == c);
        scatter(all_meas(idx), all_pred(idx), 80, 'filled', 'MarkerFaceColor', celltype_colors(c,:));
    end
