...
% This script finds all CSV files with names starting with "RD_Patient",
...
function consolidatedData = consolidateRDFiles(folderPath)
    ...
    filePattern = fullfile(folderPath, 'RD_CI*.csv');
    files = dir(filePattern);
    
    % Check if any files were found
    if isempty(files)
        error('No CSV files found with names starting with "RD_Patient".');
    end
    ...
    for i = 1:length(files)
        ...
        try
            ...
            patientID = extractBetween(files(i).name, 'RD_CI_', '_simulation');
    ...
    outputFile = fullfile(folderPath, 'Chemotherapy_Immunotherapy_Consolidated_RD_Data.csv');
    writetable(consolidatedData, outputFile);
    fprintf('Consolidated data saved to: %s\n', outputFile);
end

% Example usage:
 consolidatedData = consolidateRDFiles();