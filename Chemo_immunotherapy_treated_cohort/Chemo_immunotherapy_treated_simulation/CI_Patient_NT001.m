% $Header: svn://.../trunk/AMIGO2R2016/IMC_BC_Analysis/Chemotherapy_Immunotherapy/Chemo_Immuno_Patient_NT001.m 2410 2015-12-07 12:38:57Z evabalsa $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
inputs.pathd.results_folder='imc_bc_chemo_immunotherapy_1';         % Folder to keep results (in Results) for a given problem          
inputs.pathd.short_name='imc_chemo_immuno_p1';                      % To identify figures and reports for a given problem   
...
inputs.model.eqns = char(...
    ...
    'dM = Gr_m*M*(1-(M/K_m)) + (alpha_cd11c + alpha_cd68m)*M + k_cd45*Teff*M + k_cd31m*EC*M + k_pdpn*F*M - (k_cd163+k_cd68)*M*CC - k_ido*M*Treg - k_pdl1*M - kcarb_m*M - mu_m*M; M = max(M, 0.65)', ...
...
inputs.model.par=[0.0915 80 0.116 0.193 0.098 0.126 0.388 0.122 0.224 0.135 0.618 0.333 0.230 0.178 0.235...   % CC parameters
                 8.955 10 0.12 0.130 0.110 0.128 0.0099 0.0134 0.080 0.138 0.0905...   % Treg parameters
                 3.015 75 0.130 0.111 0.228 0.367 0.313  0.05 0.0259... % Teff parameters
                 3.10 10 0.60 0.898 0.7 0.112 0.120 0.109 0.0025...   % M parameters
                 9.7091 10 0.104 0.095...      % EC parameters
                 0.185 10 0.1696 0.064 0.0138 0.01]; % F parameters % These values may be updated during optimization  
...
%==================================
 ...
 inputs.exps.exp_y0{iexp}=[70.13 0.71 4.13 0.80 0.70 4.03];  %Initial conditions for each experiment          
...
inputs.exps.exp_data{1}=[                              
		70.13 0.71 4.13 0.80 0.70 4.03 
        %18.39 6.84 16.77 15.87 2.71 39.42 
        3.00 3.251 64.26 10.920 2.50 0.9 
		];
...
inputs.PEsol.global_theta_guess=[0.0915 0.105 6.955 0.0905 3.015 0.367 0.0259 3.10 0.0025 9.7091 0.095 0.185 0.01];
