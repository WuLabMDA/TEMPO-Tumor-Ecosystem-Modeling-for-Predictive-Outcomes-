...
function consolidatedData = consolidatePCRFiles(folderPath)
    ...
    filePattern = fullfile(folderPath, 'PCR_CI*.csv');
    files = dir(filePattern);
    
    % Check if any files were found
    if isempty(files)
        error('No CSV files found with names starting with "PCR_CI".');
    end
    ...
    for i = 1:length(files)
        ...
        try
            ...
            patientID = extractBetween(files(i).name, 'PCR_CI_', '_simulation');
    ...
    outputFile = fullfile(folderPath, 'Chemotherapy_Immunotherapy_Consolidated_PCR_Data.csv');
