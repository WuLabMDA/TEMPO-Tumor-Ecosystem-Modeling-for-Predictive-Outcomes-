...
CI_Patient_NT001   % Calls the script with the inputs: 
                           % Model
                           % Experimental scheme + data + noise
AMIGO_Prep(inputs)                            % PE problem formulation: cost function and unknowns to be estimated
...
if isfield(results, 'sim') && isfield(results.sim, 'obs')
   ...
   csv_filename = fullfile('V:\dkmarri\Mathematical_Model\AMIGO2_R2019c\AMIGO2_R2019c\IMC_BC_Analysis\Chemotherapy_Immunotherapy', 'RD_CI_Patient_001_simulation_results.csv');
